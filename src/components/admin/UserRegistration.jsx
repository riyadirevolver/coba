...
const UserRegistration = ({ data }) => {
  ...
  const handleClickUpliner = async () => {
    if (dataUpliner.length === 0 && jobLevel) {
  ...
  return (
    <DashboardCard title="Daftarin anggota karyawanmu disini!">
      <form onSubmit={handleSubmit(create)}>
              ...
              <Grid item xs={12} md={4}>
                <Box py={2}>
                  ...
                  <Autocomplete
                    ...
                    value={
                      jobLevel
                        ? data?.upliner[0].fullname
                        : data?.upliner?.fullname
                    }
        ...
        {data?.photo ?? image ? (
          ...
          >
            {loading ? <CircularProgress color="success" /> : "Tambah"}
          </Button>
        ) : (
          <Button
            variant="contained"
