...
const GenerateReportClient = ({ userId: uplinerId, token, companyId }) => {
  ...
  const { loadingNIK, nikList, setTempQuery } = useFetchUserNIK(token, {
    "$or[0][upliner2_id]": uplinerId,
    "$or[1][upliner3_id]": uplinerId,
  });
  const {
    loadingUser,
    openUser,
  ...
  const create = async (event) => {
    ...
    await generate({
      data: {
        ...
        user_id: userId,
        job_department_id: uplinerData.data.jobDepartementId,
        nik: nik,
...
export default GenerateReportClient;
