...
const EditDivisionModal = ({ open = false, closeModalHandler, data, type }) => {
  const router = useRouter();
  const { isActive, message, openSnackBar, closeSnackBar } = useSnackbar();
  const [loading, setLoading] = useState(false);
  const service = new BaseService("/api/division");
  ...
  const onEditDivision = async (event) => {
    setLoading(true);
    ...
    try {
      ...
      setLoading(false);
      openSnackBar("Berhasil Edit Divisi");
      closeModalHandler();
      ...
      console.log(error);
      setLoading(false);
      openSnackBar("Gagal Edit Divisi");
      return;
    }
  };
  return (
