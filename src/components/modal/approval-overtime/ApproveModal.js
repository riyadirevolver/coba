...
const ApproveModal = ({ open = false, closeModalHandler, type, data }) => {
  ...
  const create = async (event) => {
    try {
      ...
      setLoading(false);
      openSnackBar("Berhasil menyetujui permintaan lembur");
      closeModalHandler();
      router.reload();
    } catch (error) {
      setLoading(false);
      closeModalHandler();
      openSnackBar(
        `Gagal menyetujui permintaan lembur: ${error.response.data.message}`
      );
    }
  };
          ...
          <DialogContent>
            ...
            >
              <Typography variant="body1">
                Apakah anda ingin menyetujui permintaan lembur?
                <span
                  style={{
                    marginLeft: "5px",
                    fontWeight: 700,
